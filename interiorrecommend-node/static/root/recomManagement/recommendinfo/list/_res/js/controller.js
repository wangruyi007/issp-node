...
app.controller('infoListCtrl',function($scope,infoSer,toastr,$stateParams,$state,$location){
    ...
    function activatePage(page) {
        ...
        if($scope.infoList) return;
        infoSer.infoCount().then(function(response){
        ...
        infoSer.infoLists(listData).then(function(response){
            if(response.data.code==0){
                ...
                if($stateParams.id){
                    ...
                    angular.forEach($scope.infoList,function(obj){
   ...
   //分页
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.recomManagement.recommendinfo.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        infoSer.infoDelete(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.custom.itemsCount-count)%10){
                    ...
                    $state.go('root.recomManagement.recommendinfo.list[12]',{id:null,name:null,page:$location.search().page-1});
                }
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
