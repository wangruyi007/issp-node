var app = angular.module('recomModule', [{
    files: ['root/recomManagement/_res/js/service.js']
}]);
app.controller('recomManageCtrl', function ($scope,$state) {
    if ($state.current.url == '/recomManagement') {//默认加载列表
        $state.go('root.recomManagement.recomscheme');
    }
    $scope.$on('changeId',function(event,msg) {
        $scope.$broadcast('getId', msg)
    });
    $scope.$on('changeName',function(event,msg) {
        $scope.$broadcast('nameId', msg)
    });
    $scope.$on('page',function(event,msg){
        $scope.$broadcast('pageId',msg)
    });
}).controller('navCtrl',function($scope,$state,$location,contractsSer){
    $scope.navCla='recomscheme';
    var active = $location.path().split('/')[3];
    $scope.navCla=active?active:'recomscheme';
    ...
    $scope.showsList = [
        {id:"1",item:"内部推荐管理",menuList:[{name:'推荐方案',msg:'recomscheme'},
            {name2:"推荐类型设定",msg:'recommendtype'},
            {name3:"推荐要求设定",msg:'recommendrequire'},
            {name4:"奖励标准设定",msg:'awardstandard'},
            {name5:"推荐信息",msg:'recommendinfo'},
            {name6:"奖励信息记录",msg:'awardinfo'}],showIs:false},
