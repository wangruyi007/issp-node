var app = angular.module('workView', ['toastr']);
app.controller('workViewCtrl', function($scope,workSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    workSer.workListFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.leave.work.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.leave.work.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloInfo.length;i<len;i++){
            ...
            iframe.src=`/workDownloadFile/downloadFile${encode(obj,true)}`;
