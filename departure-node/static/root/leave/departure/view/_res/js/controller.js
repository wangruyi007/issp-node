var app = angular.module('deparView', ['toastr']);
app.controller('deparViewCtrl', function($scope,deparSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    deparSer.deparListFile(pathData).then(function(response){
        if(response.data.code== 0){
            $scope.encloInfo = response.data.data;
            $scope.length = $scope.encloInfo ? $scope.encloInfo.length : 0;
        }else{
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.encloInfo){
            for(var i = 0; i < $scope.encloInfo.length;i++){//清空所有的checked
                $scope.encloInfo[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.leave.departure.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.leave.departure.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.encloInfo){
            for(var i=0;i<$scope.encloInfo.length;i++){
                if(m===true){
                    $scope.encloInfo[i].checked=false;
                }else {
                    $scope.encloInfo[i].checked=true;
                    $scope.encloInfo[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        if(checkAllCount==$scope.encloInfo.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloInfo.length;i<len;i++){
            ...
            iframe.src=`/deparDownloadFile/downloadFile${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloInfo.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/deparDeleteFile/deleteFile',
                headers: {
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.encloInfo[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
