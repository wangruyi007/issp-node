...
app.controller('chargerAuditCtrl', function($scope, rhasAuditSer,$state,toastr,$stateParams){
    var companyId = {id : $stateParams.id};
    //获取列表
    rhasAuditSer.getOneById1(companyId).then(function(response){
    ...
    var detailId = {applyLendId:$stateParams.id};
     rhasAuditSer.auditDetailList(detailId).then(function(response){
        if(response.data.code==0){
    ...
    $scope.EditFun =function(){
        ...
        rhasAuditSer.chargerAudit(data).then(function(response){
            if(response.data.code == 0){
