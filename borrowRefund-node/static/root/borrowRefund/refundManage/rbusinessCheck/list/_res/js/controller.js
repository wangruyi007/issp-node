var app = angular.module('rbusinessCheckList', ['ng-pagination','toastr']);
app.controller('rbusinessCheckListCtrl',function($scope,rbusinessCheckSer,toastr,$stateParams,$state,$location) {
   ...
   //选择
    $scope.selectList = function(event){
        angular.forEach($scope.rbusinessCheckLists.data,function(obj){
    ...
    $scope.search = function(){
        ...
        $state.go('root.borrowRefund.refundManage.rbusinessCheck.list[12]',{reimer:$scope.reimer,reimNumber:$scope.reimNumber,page:1,startTime:startTime,endTime: endTime});
    }
    function activatePage(page) {
        ...
        rbusinessCheckSer.allCount(listData).then(function(response){
        ...
        rbusinessCheckSer.allList(listData).then(function(response){
            if(response.data.code==0){
                $scope.rbusinessCheckLists = response.data;
                if($stateParams.id){
