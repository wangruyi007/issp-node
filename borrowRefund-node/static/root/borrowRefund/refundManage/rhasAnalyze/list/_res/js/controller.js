var app = angular.module('rhasAnalyzeList', ['ng-pagination','toastr']);
app.controller('rhasAnalyzeListCtrl',function($scope,rhasAnalyzeSer,toastr,$stateParams,$state,$location) {
   ...
   //选择
    $scope.selectList = function(event){
        angular.forEach($scope.rhasAnalyzeLists.data,function(obj){
    ...
    $scope.search = function(){
        ...
        $state.go('root.borrowRefund.refundManage.rhasAnalyze.list[12]',{reimer:$scope.reimer,reimNumber:$scope.reimNumber,page:1,startTime:startTime,endTime: endTime});
    }
    ...
    function activatePage(page) {
       var startTime = angular.element('.startTime').val();
        ...
        rhasAnalyzeSer.allCount(listData).then(function(response){
        ...
        rhasAnalyzeSer.allList(listData).then(function(response){
            if(response.data.code==0){
                $scope.rhasAnalyzeLists = response.data;
                if($stateParams.id){
