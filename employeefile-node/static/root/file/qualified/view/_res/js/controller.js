var app = angular.module('qualifiedView', ['toastr']);
app.controller('qualifiedViewCtrl', function($scope,qualifiedSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    qualifiedSer.viewQualifiedFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.file.qualified.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.file.qualified.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloCompany.length;i<len;i++){
            ...
            iframe.src=`/qualified/download${encode(obj,true)}`;
