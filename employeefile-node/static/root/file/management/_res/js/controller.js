var app = angular.module('management', [{
    files:[
        "root/file/management/_res/js/service.js"
    ]
}]);
app.controller('managementCtrl',function ($scope,$state) {
    if ($state.current.url == '/management') {//默认加载列表
        $state.go('root.file.management.list[12]');
    }
}).controller('managementMenuCtrl',function($scope,$state,$rootScope,$location,managementSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        managementSer.menuPermission(buttonName).then(function(response){
