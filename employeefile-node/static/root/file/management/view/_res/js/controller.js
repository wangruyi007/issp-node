var app = angular.module('manageView', ['toastr']);
app.controller('manageViewCtrl', function($scope,managementSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    managementSer.viewManageFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.file.management.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.file.management.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloCompany.length;i<len;i++){
            ...
            iframe.src=`/management/download${encode(obj,true)}`;
