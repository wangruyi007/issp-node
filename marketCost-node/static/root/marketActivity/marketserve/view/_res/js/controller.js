...
app.controller('viewCtr', function($scope,marketserveSer,$stateParams,toastr,$http,$state,$location){
    ...
    //获取ID
    marketserveSer.viewSigning(pathData).then(function(response){
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/marketActivity/download${encode(obj,true)}`;
