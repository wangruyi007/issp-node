...
app.controller('marketserveListCtrl',function($scope,marketserveSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.marketActivity.marketserve.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        marketserveSer.marketserveapplyDel(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.marketActivity.marketserve.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
