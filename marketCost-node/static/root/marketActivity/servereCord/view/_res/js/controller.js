...
app.controller('viewCtr', function($scope,servereCordSer,$stateParams,toastr,$http,$state,$location){
    ...
    //获取ID
    servereCordSer.viewSigning(pathData).then(function(response){
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/serveRecord/download${encode(obj,true)}`;
