var app = angular.module('managementFee', [{
    files: ['root/managementFee/_res/js/service.js']
}]);
app.controller('managementFeeCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,feeSer){
    ...
    // 前面下拉导航权限
    feeSer.navPermission().then(function(response){
    ...
    // 设置导航权限
    feeSer.setPermission().then(function(response){
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"管理费用",menuList:[{name:'管理费',msg:'managementSpend'},{name2:"外包管理费",msg:'outsourcingFee'}],showIs:false},
