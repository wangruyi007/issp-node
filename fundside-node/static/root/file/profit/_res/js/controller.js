var app = angular.module('profit', [{
    files:[
        "root/file/profit/_res/js/service.js"
    ]
}]);
app.controller('profitCtrl',function ($scope,$state) {
    ...
    $scope.delete = function(){
        if($scope.idList){
            $state.go('root.file.profit.list[12]',{id:$scope.idList,name:'delete',page:$scope.page});
            $scope.menuClass = 'deleteMenu'
        }
    };
    $scope.edit = function(){
        if($scope.idList){
            $state.go('root.file.profit.edit[12]',{id:$scope.idList,page:$scope.page});
