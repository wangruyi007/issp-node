...
app.controller('recordListCtrl',function($scope,recordSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.file.record.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        recordSer.deleteContent(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.file.record.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
   ...
   //选择
    ...
    function activatePage(page) {
        ...
        recordSer.listContent(listData).then(function(response){
            if(response.data.code==0) {
    ...
    recordSer.countContent().then(function(response){
        if(response.data.code==0){
