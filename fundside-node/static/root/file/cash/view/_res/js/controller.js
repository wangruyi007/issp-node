var app = angular.module('cashView', ['toastr']);
app.controller('cashViewCtrl', function($scope,cashSer,$stateParams,toastr,$http,$state){
    ...
    cashSer.viewFiles(pathData).then(function(response){
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/cash/download${encode(obj,true)}`;
