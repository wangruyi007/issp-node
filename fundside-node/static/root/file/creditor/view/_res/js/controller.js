var app = angular.module('creditorView', ['toastr']);
app.controller('creditorViewCtrl', function($scope,creditorSer,$stateParams,toastr,$http,$state){
    ...
    creditorSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '2'){
                $state.go('root.file.creditor.view[12]',{view:'1'});
            }else if($stateParams.view == '1'){
                $state.go('root.file.creditor.view[12]',{view:'2'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/creditor/download${encode(obj,true)}`;
