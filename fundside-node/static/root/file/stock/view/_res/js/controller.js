var app = angular.module('stockView', ['toastr']);
app.controller('stockViewCtrl', function($scope,stockSer,$stateParams,toastr,$http,$state){
    ...
    stockSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '2'){
                $state.go('root.file.stock.view[12]',{view:'1'});
            }else if($stateParams.view == '1'){
                $state.go('root.file.stock.view[12]',{view:'2'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/stock/download${encode(obj,true)}`;
