var app = angular.module('transferView', ['toastr']);
app.controller('transferViewCtrl', function($scope,transferSer,$stateParams,toastr,$http,$state){
    ...
    transferSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '2'){
                $state.go('root.file.transfer.view[12]',{view:'1'});
            }else if($stateParams.view == '1'){
                $state.go('root.file.transfer.view[12]',{view:'2'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/transfer/download${encode(obj,true)}`;
