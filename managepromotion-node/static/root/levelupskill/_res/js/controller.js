var app = angular.module('bidding', [{
    files: ['root/levelupskill/_res/js/service.js']
}]);
app.controller('biddingCtrl', function ($scope,$state) {
    if ($state.current.url == '/levelupskill') {//默认加载列表
        $state.go('root.levelupskill.skills');
    }
    ...
    $scope.navCla=active?active:'skills';
    ...
    $scope.showsList = [
        {id:"1",item:"技能等级晋升管理",menuList:[{name:'技能定级',msg:'skills'},{name2:'员工职能定级',msg:'function'},{name3:'技能等级情况概览',msg:'level'},{name4:'技能晋升申请',msg:'promotion'},{name5:'员工已晋升情况',msg:'promotioned'}],showIs:false},
