var app = angular.module('car', [{
    files:[
        "root/cost/car/_res/js/service.js"
    ]
}]);
app.controller('carCtrl',function ($scope,$state) {
    ...
    $scope.delete = function(){
        if($scope.idList){
            $state.go('root.cost.car.list[12]',{id:$scope.idList,name:'delete',page:$scope.page});
    ...
    $scope.edit = function(){
        if($scope.idList){
            $state.go('root.cost.car.edit[12]',{id:$scope.idList,page:$scope.page});
    ...
    $scope.actual = function(){
        if($scope.idList){
            $state.go('root.cost.car.actual[12]',{id:$scope.idList,page:$scope.page});
