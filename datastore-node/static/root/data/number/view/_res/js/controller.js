var app = angular.module('numberView', ['toastr']);
app.controller('numberViewCtrl', function($scope,numberSer,$stateParams,toastr,$http,$state){
    ...
    numberSer.viewNumber(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.data.number.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.data.number.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloCompany.length;i<len;i++){
            ...
            iframe.src=`/number/download${encode(obj,true)}`;
