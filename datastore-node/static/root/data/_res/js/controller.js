var app = angular.module('data', [{
    files: ['root/data/_res/js/service.js']
}]);
app.controller('dataCtrl', function ($scope,$state) {
    if ($state.current.url == '/data') {//默认加载列表
         $state.go('root.data.file');
    }
    //父 Ctrl 监听到事件，向下广播
    ...
    $scope.navCla=active?active:'file';
    ...
    $scope.showsList = [
        {id:"1",item:"数据存储",menuList:[{name:'文件规范',msg:'file'},{name2:"编号规范",msg:'account'},{name3:"账号密码规范",msg:'number'}],showIs:false},
