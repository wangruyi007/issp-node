...
app.controller('demandAnalysisListCtrl',function($scope,demandAnalysisSer,toastr,$stateParams,$state){
...
//分页
    ...
    $scope.cancel = function(){ //取消删除
        $scope.delShow = false;
        $state.go('root.developProgress.market.demandAnalysis.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        demandAnalysisSer.deleteDemand(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.custom.itemsCount-count)%10){
                    ...
                    $state.go('root.developProgress.market.demandAnalysis.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
