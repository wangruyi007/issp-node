var app = angular.module('marketMiningView', ['toastr']);
app.controller('marketMiningViewCtrl', function($scope,marketMiningSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    marketMiningSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.developProgress.market.marketMining.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.developProgress.market.marketMining.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/marketchannel/downloadFile${encode(obj,true)}`;
