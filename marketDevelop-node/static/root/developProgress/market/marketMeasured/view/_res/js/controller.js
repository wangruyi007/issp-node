var app = angular.module('measuredView', ['toastr']);
app.controller('measuredViewCtrl', function($scope,measuredSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    measuredSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.developProgress.market.marketMeasured.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.developProgress.market.marketMeasured.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/marketmeasure/downloadFile${encode(obj,true)}`;
