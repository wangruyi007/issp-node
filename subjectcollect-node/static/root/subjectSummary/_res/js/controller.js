var app = angular.module('course', [{
    files: ['root/subjectSummary/_res/js/service.js']
}]);
app.controller('subjectCtrl', function ($scope,$state) {
    if ($state.current.url == '/subjectSummary') {//默认加载列表
        $state.go('root.subjectSummary.courseCollect');
    }
...
}).controller('navCtrl',function($scope,$state,$location,subjectSer){
    ...
    subjectSer.navPermission().then(function(response){
    ...
    subjectSer.setPermission().then(function(response){
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
