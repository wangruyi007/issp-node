var app = angular.module('subpackageView', ['toastr']);
app.controller('subpackageViewCtrl', function($scope,subpackageSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    subpackageSer.outFiles(pathData).then(function(response){
        if(response.data.code== 0){
            $scope.encloSubpackage = response.data.data;
            $scope.length = $scope.encloSubpackage ? $scope.encloSubpackage.length : 0;
        }else{
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.encloSubpackage){
            for(var i = 0; i < $scope.encloSubpackage.length;i++){//清空所有的checked
                $scope.encloSubpackage[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.businessContract.subpackageProject.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.businessContract.subpackageProject.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.encloSubpackage){
            for(var i=0;i<$scope.encloSubpackage.length;i++){
                if(m===true){
                    $scope.encloSubpackage[i].checked=false;
                }else {
                    $scope.encloSubpackage[i].checked=true;
                    $scope.encloSubpackage[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        var checkAllCount = 0;
        for(var i =0,len=$scope.encloSubpackage.length;i<len;i++){
            if($scope.encloSubpackage[i]['checked']){
                checkAllCount++;
            }
        }
        if(checkAllCount==$scope.encloSubpackage.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSubpackage.length;i<len;i++){
            if($scope.encloSubpackage[i].checked){
                var obj = {
                path:$scope.encloSubpackage[i].path,
                fileType:$scope.encloSubpackage[i].fileType
            };
    ...
    $scope.delFn = function(){
        ...
        var fd = new FormData();
        for(let i=0,len=$scope.encloSubpackage.length;i<len;i++){
            if($scope.encloSubpackage[i].checked){
                fd.append('paths', $scope.encloSubpackage[i].path);
                delNum.push(i);
            }
            $scope.encloSubpackage[i].checked = false;
        }
          $http({
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.encloSubpackage[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
    ...
    $scope.sortSize = function() {
        var sortArr = $scope.encloSubpackage;
