var app = angular.module('recordsView', ['toastr']);
app.controller('recordsViewCtrl', function($scope,voucherRecordsSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    voucherRecordsSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '2'){
                $state.go('root.recordAccount.voucherRecords.view[12]',{view:'1'});
            }else if($stateParams.view == '1'){
                $state.go('root.recordAccount.voucherRecords.view[12]',{view:'2'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/vouchergenerate/downloadFile${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloSigning.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/vouchergenerate/deleteFile',
                headers: {
