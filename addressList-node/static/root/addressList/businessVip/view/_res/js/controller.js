var app = angular.module('businessVipView', ['toastr']);
app.controller('businessVipViewCtrl', function($scope,businessVipSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    businessVipSer.viewFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '2'){
                $state.go('root.addressList.businessVip.view[12]',{view:'1'});
            }else if($stateParams.view == '1'){
                $state.go('root.addressList.businessVip.view[12]',{view:'2'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked){
                ...
                iframe.src=`/businessVip/downloadFile${encode(obj,true)}`;
