var app = angular.module('contractModule', [{
    files: ['root/announcement/_res/js/service.js']
}]);
app.controller('busContractCtrl', function ($scope,$state) {
    if ($state.current.url == '/announcement') {//默认加载列表
        $state.go('root.announcement.notice');
    }
    ...
    $scope.navCla = active?active:'notice';
    ...
    $scope.showsList = [
        {id:"1",item:"公告",menuList:[{name:'公告',msg:'notice'}],showIs:false},
