...
app.controller('ssuiListCtrl',function($scope,ssuiSer,toastr,$state,$stateParams,$location) {
    //获取id
    if($stateParams.id){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.projectmeasure.interface.ssui.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        ssuiSer.marketserveapplyDel(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.projectmeasure.interface.ssui.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
