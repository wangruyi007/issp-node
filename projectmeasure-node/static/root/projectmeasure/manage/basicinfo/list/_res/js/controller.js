...
app.controller('basicinfoListCtrl',function($scope,basicinfoSer,toastr,$stateParams,$state,$location) {
    //获取id
    if($stateParams.id){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.projectmeasure.manage.basicinfo.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){//确认删除
        ...
        basicinfoSer.marketserveapplyDel1(data).then(function(response){
             if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.projectmeasure.manage.basicinfo.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
