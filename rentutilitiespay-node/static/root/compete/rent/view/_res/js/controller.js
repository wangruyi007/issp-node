var app = angular.module('rentView', ['toastr']);
app.controller('rentViewCtrl', function($scope,rentSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    rentSer.rentFiles(pathData).then(function(response){
        if(response.data.code== 0){
            $scope.enclorent = response.data.data;
            $scope.length = $scope.enclorent ? $scope.enclorent.length : 0;
        }else{
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.enclorent){
            for(var i = 0; i < $scope.enclorent.length;i++){//清空所有的checked
                $scope.enclorent[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.compete.rent.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.compete.rent.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.enclorent){
            for(var i=0;i<$scope.enclorent.length;i++){
                if(m===true){
                    $scope.enclorent[i].checked=false;
                }else {
                    $scope.enclorent[i].checked=true;
                    $scope.enclorent[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        var checkAllCount = 0;
        for(var i =0,len=$scope.enclorent.length;i<len;i++){
            if($scope.enclorent[i]['checked']){
                checkAllCount++;
            }
        }
        if(checkAllCount==$scope.enclorent.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.enclorent.length;i<len;i++){
            if($scope.enclorent[i].checked){
                var obj = {
                path:$scope.enclorent[i].path,
                fileType:$scope.enclorent[i].fileType
            };
            var iframe = document.createElement('iframe');

            iframe.src=`/rentDownload/download${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        var fd = new FormData();
        for(let i=0,len=$scope.enclorent.length;i<len;i++){
            if($scope.enclorent[i].checked){
                fd.append('paths', $scope.enclorent[i].path);
                delNum.push(i);
            }
            $scope.enclorent[i].checked = false;
        }
          $http({
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.enclorent[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
