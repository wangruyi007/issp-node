var app = angular.module('Indust', [{
    files: ['root/Industrial/_res/js/service.js']
}]);
app.controller('IndustCtrl', function ($scope,$state) {
    if ($state.current.url == '/Industrial') {//默认加载列表
        $state.go('root.Industrial.registered');
    }
...
}).controller('navCtrl',function($scope,$state,$location,InduSer){
    ...
    InduSer.navPermission().then(function(response){
    ...
    InduSer.setPermission().then(function(response){
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
    ...
    $scope.showsList = [
        {id:"1",item:"工商注册年检变更",menuList:[{name:'工商注册'},{name2:"工商年检信息"},{name3:"工商税务变更"}],showIs:true},
