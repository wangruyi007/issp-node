...
app.controller('taxListCtrl',function($scope,taxSer,toastr,$stateParams,$state,$location){
   ...
   //选择
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('' +
            'root.Industrial.tax.list[12]',{id:null,name:null});
    };
        ...
        taxSer.changeDelete(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    $state.go('root.Industrial.tax.list[12]',{id:null,name:null});
                }else{
                    $state.go('root.Industrial.tax.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
