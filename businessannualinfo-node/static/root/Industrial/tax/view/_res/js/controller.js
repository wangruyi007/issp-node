var app = angular.module('taxView', ['toastr']);
app.controller('taxViewCtrl', function($scope,taxSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    taxSer.changeListFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.Industrial.tax.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.Industrial.tax.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloMaterial.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/changeDeleteFile/deleteFile',
                headers: {
