var app = angular.module('registeredView', ['toastr']);
app.controller('registeredViewCtrl', function($scope,registeredSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    registeredSer.loginListFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.Industrial.registered.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.Industrial.registered.view[12]',{view:'1'});
            }
        }
    };
