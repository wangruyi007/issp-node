...
app.controller('moneyListCtrl',function($scope,moneySer,toastr,$stateParams,$state){
    //获取id
    if($stateParams.id){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.assessment.money.list[12]',{id:null,name:null});
    };
    $scope.delYes = function(){
        var data = {
            id :$stateParams.id
        };
        moneySer.deleteMoney(data).then(function(response){
