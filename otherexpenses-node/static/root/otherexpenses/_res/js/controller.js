var app = angular.module('otherexpenses', [{
    files: ['root/otherexpenses/_res/js/service.js']
}]);
app.controller('otherexpensesCtrl', function ($scope,$state) {
...
}).controller('navCtrl',function($scope,$state,$location,otherexpensesSer){
    ...
    otherexpensesSer.navPermission().then(function(response){
    ...
    otherexpensesSer.setPermission().then(function(response){
        if(response.data.code == 0){
            var data = response.data.data;
            if(data && data.length){
                $scope.isHide = false;
