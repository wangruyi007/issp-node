var app = angular.module('infoView', ['toastr']);
app.controller('infoViewCtrl', function($scope,infoSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    infoSer.infoFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.biddingManagement.biddingInformation.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.biddingManagement.biddingInformation.view[12]',{view:'1'});
            }
        }
    };
