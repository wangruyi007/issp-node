...
app.controller('MaterialListCtrl',function($scope,MaterialSer,toastr,$stateParams,$state,$location){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('' +
            'root.biddingManagement.tenderMaterial.list[12]',{id:null,name:null});
    };
    var count=0;
        ...
        MaterialSer.deleteSource(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.custom.itemsCount-count)%10){
                    $state.go('root.biddingManagement.tenderMaterial.list[12]',{id:null,name:null});
                }else{
                    $state.go('root.biddingManagement.tenderMaterial.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
                // $state.go('root.biddingManagement.tenderMaterial.list[12]',{id:null,name:null});
            }else{
