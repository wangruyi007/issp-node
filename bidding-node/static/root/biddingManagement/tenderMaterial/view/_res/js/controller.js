var app = angular.module('MaterialView', ['toastr']);
app.controller('MaterialViewCtrl', function($scope,MaterialSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    MaterialSer.MaterialFiles(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.biddingManagement.tenderMaterial.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.biddingManagement.tenderMaterial.view[12]',{view:'1'});
            }
        }
    };
