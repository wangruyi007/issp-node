var app = angular.module('questionView', ['toastr']);
app.controller('questionViewCtrl', function($scope,questionSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    questionSer.questionFiles(pathData).then(function(response){
        if(response.data.code== 0){
            $scope.encloQuestion = response.data.data;
            $scope.length = $scope.encloQuestion ? $scope.encloQuestion.length : 0;
        }else{
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.encloQuestion){
            for(var i = 0; i < $scope.encloQuestion.length;i++){//清空所有的checked
                $scope.encloQuestion[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.biddingManagement.tenderQuestion.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.biddingManagement.tenderQuestion.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.encloQuestion){
            for(var i=0;i<$scope.encloQuestion.length;i++){
                if(m===true){
                    $scope.encloQuestion[i].checked=false;
                }else {
                    $scope.encloQuestion[i].checked=true;
                    $scope.encloQuestion[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        if(checkAllCount==$scope.encloQuestion.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
          ...
          $http({
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.encloQuestion[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
    ...
    $scope.sortSize = function() {
        var sortArr = $scope.encloQuestion;
