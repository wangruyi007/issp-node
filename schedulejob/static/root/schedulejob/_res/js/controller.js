...
app.controller('schedulejobCtrl', function ($scope,$state) {
    if ($state.current.url == '/schedulejob') {//默认加载列表
         $state.go('root.schedulejob.mission');
    }
    //父 Ctrl 监听到事件，向下广播
    ...
    $scope.navCla='mission';
    var active = $location.path().split('/')[3];
    $scope.navCla=active?active:'mission';
    $scope.navClass= function(name){
        $scope.navCla=name
    }
    //下拉菜单
    $scope.showsList = [
        {id:"1",item:"任务调度业务",menuList:[{name:'任务调度'},{name2:'任务调度组'}],showIs:false}
    ];
