var app = angular.module('out', [{
    files: ['root/out/_res/js/service.js']
}]);
app.controller('outCtrl', function ($scope,$state) {
    if ($state.current.url == '/out') {//默认加载列表
         $state.go('root.out.capital');
    }
    //父 Ctrl 监听到事件，向下广播
    ...
    $scope.showsList = [
        {id:"1",item:"出车费资金准备与支付",menuList:[{name:'资金准备审核',msg:'capital'},{name2:"等待付款",msg:'wait'},{name3:"已付款",msg:'has'}],showIs:false},
