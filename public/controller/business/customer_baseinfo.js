...
function editBtn(self){
    if($('section.selected').length > 0){
        ...
        $('.content-input.edit .customerLevelTO option[value="' + data.customerLevelTO + '"]').attr("selected", true);
...
var basicCount=function(){
    $.get('/customer/customerbaseinfo/count',function(data){
        var count = data.data;
    ...
    if(data.data===null){
        ...
        $.each(data.data, function(i, item){
            ...
            "<div class='frozen-m'><button onclick='thaw(this)'>解冻</button></div></section>").format({
                ...
                customerLevelTO : item.customerLevelVO.name==null?"":item.customerLevelVO.name,
