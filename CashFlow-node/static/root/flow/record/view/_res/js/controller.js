var app = angular.module('recordView', ['toastr']);
app.controller('recordViewCtrl', function($scope,recordSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    recordSer.moneyListFile(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.flow.record.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.flow.record.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloMaterial.length;i<len;i++){
            ...
            iframe.src=`/moneyDownloadFile/downloadFile${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        for(let i=0,len=$scope.encloMaterial.length;i<len;i++){
          ...
          $http({
                method: 'POST',
                url: '/moneyDeleteFile/deleteFile',
                headers: {
