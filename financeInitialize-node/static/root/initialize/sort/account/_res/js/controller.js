var app = angular.module('account', [{
    files:[
        "root/initialize/sort/account/_res/js/service.js"
    ]
}]);
app.controller('accountCtrl',function ($scope,$state) {
    ...
    $scope.delete = function(){
        if($scope.idList){
            $state.go('root.initialize.sort.account.list.delete[12]',{id:$scope.idList,name:'delete',page:$scope.page});
    ...
    $scope.edit = function(){
        if($scope.idList){
            $state.go('root.initialize.sort.account.edit[12]',{id:$scope.idList});
