var app = angular.module('commonList', ['ng-pagination','toastr']);
app.controller('commonListCtrl',function($scope,assetsSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.initialize.sort.common.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        assetsSer.marketserveapplyDel1(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.initialize.sort.common.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
