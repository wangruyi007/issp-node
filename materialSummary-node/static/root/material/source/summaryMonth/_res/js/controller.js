...
app.controller('summaryMonthCtrl', function($scope, sourSer,toastr){
    $scope.showed=true;
    ...
    $scope.collect = function(){
        ...
        sourSer.sourSummaryMonth(vm.sum).then(function(response){
            if(response.data.code == 0){
                ...
                for(var i = 0;i < $scope.data.length;i++){
                    if (i == 0){
                        $scope.arr[index] = {};
                        $scope.arr[index].name = $scope.data[i].sources;
                        $scope.arr[index].length = 1;
                    }
                    if(i !== 0){
                        if($scope.data[i].sources == before){
                            $scope.arr[index].length +=1;
                        }else if($scope.data[i].sources == '合计'){
                            $scope.data[i].area=' ';
                            index +=1;
                            $scope.arr[index] = {};
                            $scope.arr[index].name = '合计';
                            $scope.arr[index].length = 1;
                        }else if($scope.data[i].sources != before){
                            index +=1;
                            $scope.arr[index] = {};
                            $scope.arr[index].name = $scope.data[i].sources;
                            $scope.arr[index].length = 1;
