var app = angular.module('source', [{
    files:[
        "root/material/source/_res/js/service.js",
    ]
}]);
app.controller('sourCtrl',function ($scope,$state) {
    if ($state.current.url == '/source') {//默认加载列表
        $state.go('root.material.source.summaryDay[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('sourMenuCtrl',function($scope,$state,$rootScope,$location,sourSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        sourSer.subsPermission(buttonName).then(function(response){
