var app = angular.module('maintain', [{
    files:[
        "root/material/maintain/_res/js/service.js",
    ]
}]);
app.controller('mainCtrl',function ($scope,$state) {
    if ($state.current.url == '/maintain') {//默认加载列表
        $state.go('root.material.maintain.summaryDay[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('mainMenuCtrl',function($scope,$state,$rootScope,$location,mainSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        mainSer.subsPermission(buttonName).then(function(response){
