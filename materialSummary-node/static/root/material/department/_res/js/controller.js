var app = angular.module('department', [{
    files:[
        "root/material/department/_res/js/service.js",
    ]
}]);
app.controller('departCtrl',function ($scope,$state) {
    if ($state.current.url == '/department') {//默认加载列表
        $state.go('root.material.department.summaryDay[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('departMenuCtrl',function($scope,$state,$rootScope,$location,departSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        departSer.subsPermission(buttonName).then(function(response){
