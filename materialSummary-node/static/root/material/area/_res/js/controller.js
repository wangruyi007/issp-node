var app = angular.module('area', [{
    files:[
        "root/material/area/_res/js/service.js",
    ]
}]);
app.controller('subsCtrl',function ($scope,$state) {
    if ($state.current.url == '/area') {//默认加载列表
        $state.go('root.material.area.summaryDay[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('subsMenuCtrl',function($scope,$state,$rootScope,$location,areaSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        areaSer.subsPermission(buttonName).then(function(response){
