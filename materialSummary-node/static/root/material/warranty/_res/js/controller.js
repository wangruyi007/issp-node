var app = angular.module('warranty', [{
    files:[
        "root/material/warranty/_res/js/service.js",
    ]
}]);
app.controller('warCtrl',function ($scope,$state) {
    if ($state.current.url == '/warranty') {//默认加载列表
        $state.go('root.material.warranty.summaryDay[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('warMenuCtrl',function($scope,$state,$rootScope,$location,warSer){
    ...
    $scope.menuCheck = function (name) {
        ...
        warSer.subsPermission(buttonName).then(function(response){
