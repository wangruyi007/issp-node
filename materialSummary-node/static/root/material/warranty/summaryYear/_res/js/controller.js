...
app.controller('summaryYearCtrl', function($scope, warSer,toastr){
    $scope.showed=true;
    $scope.collect = function(){
        ...
        warSer.warSummaryYear(data.sum).then(function(response){
            if(response.data.code == 0){
                ...
                for(var i = 0;i < $scope.data.length;i++){
                    if (i == 0){
                        $scope.arr[index] = {};
                        $scope.arr[index].name = $scope.data[i].warranty;
                        $scope.arr[index].length = 1;
                    }
                    if(i !== 0){
                        if($scope.data[i].warranty == before){
                            $scope.arr[index].length +=1;
                        }else if($scope.data[i].warranty == '合计'){
                            $scope.data[i].area=' ';
                            index +=1;
                            $scope.arr[index] = {};
                            $scope.arr[index].name = '合计';
                            $scope.arr[index].length = 1;
                        }else if($scope.data[i].warranty != before){
                            index +=1;
                            $scope.arr[index] = {};
                            $scope.arr[index].name = $scope.data[i].warranty;
                            $scope.arr[index].length = 1;
