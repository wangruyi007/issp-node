var app = angular.module('alreadyList', ['ng-pagination','toastr']);
app.controller('alreadyListCtrl',function($scope,alreadySer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.bonus.already.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){//确认删除
        ...
        alreadySer.deleteAlready(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.bonus.already.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
   ...
   //选择
    $scope.selectList = function(event){
        angular.forEach($scope.alreadyLists.data,function(obj){
