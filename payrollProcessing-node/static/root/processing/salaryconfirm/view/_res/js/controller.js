var app = angular.module('salaryconfirmView', ['toastr']);
app.controller('salaryconfirmViewCtrl', function($scope,salaryconfirmSer,$stateParams,toastr,$http,$state){
    ...
    //获取附件列表
    salaryconfirmSer.salaryconfirmFiles(pathData).then(function(response){
        if(response.data.code== 0){
            $scope.encloSsui = response.data.data;
            $scope.length = $scope.encloSsui ? $scope.encloSsui.length : 0;
        }else{
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.encloSsui){
            for(var i = 0; i < $scope.encloSsui.length;i++){//清空所有的checked
                $scope.encloSsui[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.processing.salaryconfirm.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.processing.salaryconfirm.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.encloSsui){
            for(var i=0;i<$scope.encloSsui.length;i++){
                if(m===true){
                    $scope.encloSsui[i].checked=false;
                }else {
                    $scope.encloSsui[i].checked=true;
                    $scope.encloSsui[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        var checkAllCount = 0;
        for(var i =0,len=$scope.encloSsui.length;i<len;i++){
            if($scope.encloSsui[i]['checked']){
                checkAllCount++;
            }
        }
        if(checkAllCount==$scope.encloSsui.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSsui.length;i<len;i++){
            if($scope.encloSsui[i].checked){
                var obj = {
                path:$scope.encloSsui[i].path,
                fileType:$scope.encloSsui[i].fileType
            };
            console.log(obj)
            var iframe = document.createElement('iframe');

            iframe.src=`/salaryconfirmDownload/download${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        var fd = new FormData();
        for(let i=0,len=$scope.encloSsui.length;i<len;i++){
            if($scope.encloSsui[i].checked){
                console.log($scope.encloSsui[i].path)
                fd.append('paths', $scope.encloSsui[i].path);
                delNum.push(i);
            }
            $scope.encloSsui[i].checked = false;
        }
          $http({
                method: 'POST',
                url: '/salaryconfirmDelfile/delfile',
                headers: {
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.encloSsui[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
