var app = angular.module('processedModule', [{
    files: ['root/processing/_res/js/service.js']
}]);
app.controller('processedCtrl', function ($scope,$state) {
    if ($state.current.url == '/processing') {//默认加载列表
        $state.go('root.processing.waitconfirm');
    }
...
}).controller('navsCtrl',function($scope,$state,$location,proceSer){
    ...
    $scope.showsList = [
        {id:"1",item:"薪资核算确认业务",menuList:[{name:'待确认薪资'},{name2:"已付薪资"},{name3:"薪资核算确认业务接口"},{name4:"汇总分析"},{name5:"已确认薪资"}],showIs:true},
        {id:"2",item:"发票管理业务",menuList:[{name6:'上交发票'}],showIs:true},
        {id:"3",item:"设置",menuList:[{name7:'设置'}],showIs:true}
    ];
