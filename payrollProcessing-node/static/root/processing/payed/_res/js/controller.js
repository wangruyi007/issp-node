var app = angular.module('payed', [{
    files:[
        "root/processing/payed/_res/js/service.js"
    ]
}]);
app.controller('payedCtrl',function ($scope,$state) {
    if ($state.current.url == '/payed') {
        $state.go('root.processing.payed.list[12]');
    }
    ...
    $scope.first = function(){
        if($scope.idList){
            $state.go('root.processing.payed.list[12]',{id:$scope.idList,name:'first',page:$scope.page});
            $scope.menuClass = 'firstMenu'
        }
    };
    $scope.second = function(){
        if($scope.idList){
            $state.go('root.processing.payed.list[12]',{id:$scope.idList,name:'second',page:$scope.page});
