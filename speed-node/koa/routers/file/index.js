...
module.exports = function(){
    ...
    }).get('/resources/guidePermission/:guideAddrStatus', function*(){  //参考指标导航权限
        var $self = this;
        var page = {name:$self.params.guideAddrStatus,token:$self.cookies.get('token')};
        yield (server().resPermission(page)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/resources/list', function*(){  //参考指标列表
        var $self = this;
    ...
    }).get('/resources/getOneById', function*(){
        var $self = this;
        var editId = $self.request.query;
        editId.token = $self.cookies.get('token');
        yield (server().resEditById(editId)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
