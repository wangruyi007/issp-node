var app = angular.module('materialModule', [{
    files: ['root/materialBuy/_res/js/service.js']
}]);
app.controller('materialCtrl', function ($scope,$state) {
    if ($state.current.url == '/materialBuy') {//默认加载列表
        $state.go('root.materialBuy.deviceType');
    }
    ...
    $scope.navCla='deviceType';
    var active = $location.path().split('/')[3];
    $scope.navCla=active?active:'deviceType';
