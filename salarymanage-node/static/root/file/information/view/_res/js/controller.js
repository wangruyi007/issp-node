var app = angular.module('informationView', ['toastr']);
app.controller('informationViewCtrl', function($scope,informationSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    informationSer.viewCompany(pathData).then(function(response){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            if($scope.encloCompany){
            for(var i = 0; i < $scope.encloCompany.length;i++){//清空所有的checked
                $scope.encloCompany[i].checked = false;
            }
            if($stateParams.view == '1'){
                $state.go('root.file.information.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.file.information.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.all= function (m) {//全选功能
        ...
        if($scope.encloCompany){
            for(var i=0;i<$scope.encloCompany.length;i++){
                if(m===true){
                    $scope.encloCompany[i].checked=false;
                }else {
                    $scope.encloCompany[i].checked=true;
                    $scope.encloCompany[i].checkedSingle=true;
                }
            }
        }
    };
    ...
    $scope.checkedFun = function (obj) {
        ...
        var checkAllCount = 0;
        for(var i =0,len=$scope.encloCompany.length;i<len;i++){
            if($scope.encloCompany[i]['checked']){
                checkAllCount++;
            }
        }
        if(checkAllCount==$scope.encloCompany.length){
            $scope.checked= true;
        }else{
            $scope.checked=false;
        }
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloCompany.length;i<len;i++){
            if($scope.encloCompany[i].checked){
                var obj = {
                path:$scope.encloCompany[i].path,
                fileType:$scope.encloCompany[i].fileType
            };
            var iframe = document.createElement('iframe');

            iframe.src=`/information/download${encode(obj,true)}`;
    ...
    $scope.delFn = function(){
        ...
        var fd = new FormData();
        for(let i=0,len=$scope.encloCompany.length;i<len;i++){
            if($scope.encloCompany[i].checked){
                fd.append('paths', $scope.encloCompany[i].path);
                delNum.push(i);
            }
            $scope.encloCompany[i].checked = false;
        }
          $http({
                ...
                if (response.data.code == 0) {
                    ...
                    for(var i = 0;i<delNum.length;i++){
                        $scope.encloCompany[delNum[i]].delel = true;
                        $scope.length--;
                    }
                    if(!$scope.length){
    ...
    $scope.sortSize = function() {
        var sortArr = $scope.encloCompany;
