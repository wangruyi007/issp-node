...
app.controller('informationEditCtrl', function($scope, informationSer,$state,toastr,$stateParams){
    ...
    $scope.firstHot = {};
    $scope.firstttached = {};
    informationSer.getOneById(companyId).then(function(response){
        if(response.data.code==0){
            $scope.add = response.data.data;
            $scope.firstBasic.area=$scope.add.area;
            $scope.firstBasic.name=$scope.add.employeeName;
            $scope.firstBasic.department=$scope.add.section;
            $scope.firstBasic.position=$scope.add.station;
            $scope.firstBasic.entryTime=$scope.add.hiredate;
            $scope.firstBasic.entryTime=$scope.add.hiredate;
            $scope.firstManage.currentLevel=$scope.add.manageLevel;
            $scope.firstWorker.data=$scope.add.positiveTime;
            $scope.firstComputer.assistMoney=$scope.add.computerSubsidies;
            $scope.firstHouse.money=$scope.add.accommodationSubsidies;
            $scope.firstAge.assisCondition=$scope.add.senioritySubsidies;
            $scope.firstHot.moneyCondition=$scope.add.hyperthermiaSubsidies;
            $scope.firstttached.money=$scope.add.jinpoCost;
        }else{
    ...
    $scope.basicEditFun = function(){
        ...
        informationSer.editContent(data).then(function(response){
            if(response.data.code == 0){
