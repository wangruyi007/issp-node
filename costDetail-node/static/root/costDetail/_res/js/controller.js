var app = angular.module('contractModule', [{
    files: ['root/costDetail/_res/js/service.js']
}]);
app.controller('busContractCtrl', function ($scope,$state) {
    if ($state.current.url == '/costDetail') {//默认加载列表
        $state.go('root.costDetail.detail');
    }
    ...
    $scope.navCla='detail';
    var active = $location.path().split('/')[3] == 'setting'?$location.path().split('/')[3]:$location.path().split('/')[4];
    ...
    $scope.showsList = [
        {id:"1",item:"成本明细管理",menuList:[{name:'成本明细',msg:'detail'},],showIs:false},
        {id:"2",item:"设置明细",menuList:[{name2:'设置分类明细',msg:'classify'},],showIs:false},
