...
app.controller('taxesmanageListCtrl',function($scope,taxesmanageSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.taxes.taxes.taxesmanage.list[12]',{id:null,name:null});
    };
    var count = 0;
        ...
        taxesmanageSer.marketserveapplyDel1(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.taxes.taxes.taxesmanage.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
