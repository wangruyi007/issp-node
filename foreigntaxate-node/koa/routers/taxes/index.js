...
module.exports = function(){
    ...
    //列表 税金管理
    router.get('/taxes/setButtonPermission', function*(){ //设置导航权限
        var $self = this;
    ...
    }).get('/taxes/sonPermission', function*(){ //菜单导航权限
        var $self = this;
        var navToken = {userToken:$self.cookies.get('token')};
    ...
    }).get('/taxmanagement/guidePermission/:guideAddrStatus', function*(){ //税金管理 功能菜单权限
        var $self = this;
        var page = {name:$self.params.guideAddrStatus,userToken:$self.cookies.get('token')};
        yield (server().tMPermission(page)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/taxes/taxmanagement/list', function*(){
       var $self = this;
       var page = this.request.query;
       page.userToken = this.cookies.get('token');
        yield (server(page).taxesList(page)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/taxes/taxmanagement/getCompany', function*(){//获取汇总的所有公司
        var $self = this;
        yield (server().getCompany()
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/taxes/taxmanagement/add', function*(){//添加
        var $self = this;
    ...
    }).post('/taxmanagement/upload', koaBody({multipart:true}),function *(next) {//上传文件 税金管理
        var $self = this;
        var uploadData = $self.request.body;
        uploadData.userToken = $self.cookies.get("token");
        yield (server().taxmanagementUploadFile(uploadData)
            .then((parsedBody) =>{
    ...
    }).get('/taxmanagement/viewFile', function*(){ //查看附件 税金管理
        var $self = this;
        var enData = $self.request.query;
        enData.userToken = $self.cookies.get('token');
        yield (server().taxmanagementSeeFile(enData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/taxmanagement/delFile', koaBody({multipart:true}), function*(){//税金管理 删除文件
        var $self = this;
        var delData = $self.request.body;
        delData.userToken = $self.cookies.get('token');
        yield (server().taxmanagementDelFile(delData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/taxmanagement/download', function*(){//下载文件 税金管理
        var $self = this;
        var count = $self.request.query;
        var data = {
            path:count.path
        };
        yield (fetch(config()['rurl']+`/taxmanagement/v1/downloadFile${urlEncode(data,true)}`, {
            method : 'GET',
    ...
    }).get('/account/guidePermission/:guideAddrStatus', function*(){ //对外资料管理 功能菜单权限
        var $self = this;
        var page = {name:$self.params.guideAddrStatus,userToken:$self.cookies.get('token')};
    ...
    }).post('/account/upload', koaBody({multipart:true}),function *(next) {//上传文件 税金管理
        var $self = this;
        var uploadData = $self.request.body;
        uploadData.userToken = $self.cookies.get("token");
        yield (server().accountUploadFile(uploadData)
            .then((parsedBody) =>{
                $self.body = parsedBody;
            }).catch((error) =>{
                $self.set('Content-Type','application/json;charset=utf-8');
    ...
    }).get('/account/viewFile', function*(){ //查看附件 税金管理
        var $self = this;
        var enData = $self.request.query;
        enData.userToken = $self.cookies.get('token');
        yield (server().accountSeeFile(enData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).post('/account/delFile', koaBody({multipart:true}), function*(){//税金管理 删除文件
        var $self = this;
        var delData = $self.request.body;
        delData.userToken = $self.cookies.get('token');
        yield (server().accountDelFile(delData)
            .then((parsedBody) =>{
                var responseText = JSON.parse(parsedBody);
                $self.body = responseText;
            }).catch((error) =>{
    ...
    }).get('/account/download', function*(){//下载文件 税金管理
        var $self = this;
        var count = $self.request.query;
        var data = {
            path:count.path
        };
        yield (fetch(config()['rurl']+`/accountinfomanagement/v1/downloadFile${urlEncode(data,true)}`, {
            method : 'GET',
            headers : {'userToken' : $self.cookies.get('token')}
        }).then((res)=>{
            fileType(count,this);
