var app = angular.module('mineindexList', ['ng-pagination','toastr']);
app.controller('mineindexListCtrl',function($scope,mineindexSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.file.mineindex.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        mineindexSer.deleteContent(data).then(function(response){
            if(response.data.code==0){
                count++;
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.file.mineindex.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
   ...
   //选择
    ...
    function activatePage(page) {
        ...
        mineindexSer.countContent(listData).then(function(response){
            if(response.data.code==0){
        ...
        mineindexSer.listContent(listData).then(function(response){
            if(response.data.code==0) {
