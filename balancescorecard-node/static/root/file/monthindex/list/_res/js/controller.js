var app = angular.module('monthindexList', ['ng-pagination','toastr']);
app.controller('monthindexListCtrl',function($scope,monthindexSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.file.monthindex.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        monthindexSer.deleteContent(data).then(function(response){
            if(response.data.code==0){
                count++;
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.file.monthindex.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
   ...
   //选择
    ...
    function activatePage(page) {
        ...
        monthindexSer.countContent(listData).then(function(response){
            if(response.data.code==0){
        ...
        monthindexSer.listContent(listData).then(function(response){
            if(response.data.code==0) {
