var app = angular.module('departindexList', ['ng-pagination','toastr']);
app.controller('departindexListCtrl',function($scope,departindexSer,toastr,$stateParams,$state,$location) {
    ...
    $scope.cancel = function(){
        $scope.delShow = false;
        $state.go('root.file.departindex.list[12]',{id:null,name:null});
    };
    var count = 0;
    $scope.delFn = function(){
        ...
        departindexSer.deleteContent(data).then(function(response){
            if(response.data.code==0){
                count++;
                ...
                if(($scope.abili.itemsCount-count)%10){
                    ...
                    $state.go('root.file.departindex.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
    ...
    //搜索
    if($stateParams.department){$scope.department = $stateParams.department;$scope.isView = false;}
   ...
   //选择
    ...
    function activatePage(page) {
        ...
        departindexSer.countContent(listData).then(function(response){
            if(response.data.code==0){
        ...
        departindexSer.listContent(listData).then(function(response){
            if(response.data.code==0) {
