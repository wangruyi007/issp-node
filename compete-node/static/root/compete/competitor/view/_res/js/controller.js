...
app.controller('competitorViewCtr', function($scope,competitorSer,$stateParams,toastr,$http,$state){
    ...
    //获取ID
    competitorSer.viewSigning(pathData).then(function(response){
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            ...
            iframe.src=`/compete/download${encode(obj,true)}`;
