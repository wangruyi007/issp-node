...
app.controller('emailCtrl',function ($scope,$state) {
    ...
    $scope.congeal = function(){
        if($scope.idList){
            $state.go('root.compete.email.list.congeal[12]',{id:$scope.idList});
        }
    };
    ...
    $scope.delete = function(){
        if($scope.idList){
            $state.go('root.compete.email.list.delete[12]',{id:$scope.idList});
    ...
    //解冻
     $scope.unfreeze = function(){
        if($scope.idList){
            $state.go('root.compete.email.list.unfreeze[12]',{id:$scope.idList});
