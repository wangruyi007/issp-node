...
app.controller('waitListCtrl',function($scope,waitSer,toastr,$stateParams,$state,$location){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('' +
            'root.equipment.wait.list[12]',{id:null,name:null});
    };
    var count=0;
        ...
        waitSer.deleteWait(data).then(function(response){
            if(response.data.code==0){
                ...
                if(($scope.custom.itemsCount-count)%10){
                    $state.go('root.equipment.wait.list[12]',{id:null,name:null});
                }else{
                    $state.go('root.equipment.wait.list[12]',{id:null,name:null,page:$stateParams.page-1});
                }
            }else{
